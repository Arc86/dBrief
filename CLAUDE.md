...
- `TranscriptAnalysisView` loads it on open and, on Save, rewrites the sidecar **and** surgically updates the existing Markdown via `MarkdownInsightsUpdater.update(markdown:with:)` — replacing only the `## 📝 Summary` / `## ✅ Action Items` / `## 🏷️ Tags` sections and the frontmatter `tags:` list, leaving the transcript and other content untouched. A Save does **not** auto-re-dispatch to integrations (avoids duplicate notes/reminders) — use **Re-export** for that (see below).

### Pushing post-recording edits downstream

Edits made in the transcript viewer after the pipeline ran (speaker renames → `.richtranscript.json`; analysis edits → `.insights.json`) don't otherwise reach the generated Markdown or the integration destinations. Two mechanisms close that gap:

- **Re-export** (toolbar button in `TranscriptDetailView`) → `RecordingManager.reexport(recording:richTranscript:insights:)`: applies the current edited rich transcript + insights onto the recording, regenerates the Markdown **in place** (overwrites the file at `RecordingInsights.markdownPath` via `MarkdownGenerator.write(recording:to:…)`, so the monitored/Obsidian path stays stable even if the title changed), refreshes the insights sidecar, then **re-dispatches to every enabled integration**. A confirmation dialog warns that Apple Notes / Notion / Reminders create *new* entries (no update-in-place); results show per destination.
- **Hold for review before processing** (`AppSettings.holdForReviewBeforeProcessing`, Settings → Recording): after transcription, `processRecording` parks the recording behind a `.review.json` marker (separate from the `.queue.json` batch queue so "Process Queue" never grabs it), sets `AppState.shouldOpenTranscriptForReview`, and stops before AI/Markdown/dispatch. `MenuBarView` opens the transcript window; the user renames speakers, then **"Process now"** (`processNowAfterReview` → `retryAIAnalysis`, which now also loads the rich-transcript sidecar) runs AI → Markdown → a single dispatch — so the only export already reflects the edits, with no duplicates. `RecordingManager.isAwaitingReview(recording:)` gates the "Process now" affordances in `TranscriptDetailView` and `RecordingHistoryView`.

`MarkdownGenerator` is split into `render(...) -> String` (pure, testable), `defaultOutputURL(for:in:)`, and `write(recording:to:…)`; `generate(...)` composes the latter two. `RecordingManager.currentModelEndpoints()` centralizes the engine→frontmatter-model mapping shared by the initial pipeline, retry, and re-export.
