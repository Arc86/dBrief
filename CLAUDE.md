...
**Live input-device hot-swap**: `AudioCaptureManager.switchMicrophoneDevice(to:)` changes the mic input device mid-recording (pause engine → remove tap → `AudioInputDeviceManager.applyInputDevice` → re-install tap → resume) while keeping the single in-progress mic CAF open. If the new device's format differs from the file's established format (`AudioTrackWriter.establishedFormat`), buffers are converted via `MicFormatConverter` so the track stays valid. Driven manually from the mic menu in `RecordingControlsView` via `RecordingManager.switchInputDevice(to:)` (no auto-detect listener).
...
After transcription, **all** engines' results pass through the engine-agnostic `TranscriptCleanup` (in `dBriefWire`, called once in `RecordingManager.transcribeRecordingAudio`): it strips hallucination/markup noise (XML-ish tags, `[…]`/`{…}` non-speech annotations) and normalizes whitespace always, and removes filler words (um, uh, …) only when `AppSettings.removeFillerWords` is on (default off; **Settings → Transcription → Cleanup**).
...
| Remote Endpoint | `AIService` | OpenAI-compatible `/v1/chat/completions` (OpenAI, **Groq**, **Gemini** via Google's OpenAI-compatible endpoint, Ollama — all presets) plus native **Anthropic** `/v1/messages` (`x-api-key` + `anthropic-version`, `content[0].text` / `content_block_delta` streaming) selected by `Endpoint.provider`. For recognized reasoning models, `ReasoningConfig` merges chain-of-thought-suppressing params (`reasoning_effort`, `reasoning_format: hidden`, `chat_template_kwargs.enable_thinking`) into the OpenAI-compatible body. Endpoints are added via the curated `ProviderPresets` picker in Settings → AI Analysis. |
...
**Custom vocabulary in AI prompts**: the user's existing custom-vocabulary string (`AppSettings.effectiveWhisperPrompt`, which also biases Whisper) is injected as a "spell these exactly" `### DOMAIN-SPECIFIC TERMS` block into every AI analysis path — `UnifiedInsightsPrompt.vocabularyBlock` / the `customVocabulary` params on `systemPrompt(…)` and `systemPromptForGuidedGeneration(…)` for the unified engines (threaded over the wire to the Gemma helper via `MLRequest.analyze`/`.analyzeStream`), and appended to the remote per-task system prompts in `RecordingManager.withVocabulary`.
