...
- **Post-hoc speaker assignment**: Clicking a speaker label on a turn card in `TranscriptWindowView` opens `SpeakerAssignPicker`, a popover with two independent actions, both applied by the pure, unit-tested `SpeakerReassignment` (`Services/SpeakerReassignment.swift`) and persisted via `TranscriptStore` (`.richtranscript.json` sidecar):
  - **Rename speaker** — a free-text field (prefilled with the current name, so any speaker incl. a raw "Speaker N" can be named) plus one-tap **suggestions** of known names not yet a speaker (`recording.participants` + calendar attendees). `SpeakerReassignment.rename(_:speakerId:to:)` edits the speaker's `SpeakerLabel.displayName`, keeping its `speakerId` and all segments. If the new name (case-insensitively) already belongs to **another** speaker, the two **swap** display names — the fix for diarization mixing up who's who; no speaker is merged or lost. Applies to the whole speaker.
  - **Move this turn to** — lists the *other* existing speakers; choosing one reassigns segments via `SpeakerReassignment.apply(.existing(...))` with a scope step (**This turn** vs **All N from "<name>"**; skipped when the turn already is the whole speaker). `apply` rewrites `RichSegment.speakerId` over the chosen segment ids, drops orphaned `SpeakerLabel`s (moving *all* of a speaker's segments thus merges it into the target), and transfers `meSpeakerId` if its speaker is fully merged away.
...
- **`SpeakerPillView`** — colored speaker badge (dead code; speaker assignment is triggered from the turn card speaker label in `TranscriptWindowView` via `SpeakerAssignPicker`)
