...
- **SettingsView** — sidebar-based settings window with tabs:
  ...
  - **AI & Models** (`SettingsAIModelsTab`) — two sub-tabs: **Transcription** (`SettingsTranscriptionTab`: engine; for Apple Speech an `ApplePrivacyBadge` privacy indicator (on-device on macOS 26+ via `SpeechAnalyzer`, may use Apple's servers on macOS 14–25 — also shown in onboarding); for Local Whisper a model **card** with Recommended badge + per-model plain-language descriptor + `ModelDownloadButton`, a help popover, diarization, and an **Advanced** disclosure holding compute units, "Show all models", refresh, and purge; plus language, custom vocabulary, endpoints, chunking) and **AI Analysis** (`SettingsAITab`: AI engine, Gemma model download, prompts, AI processing toggle, output language, endpoints)
