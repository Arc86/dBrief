...
### Speaker Recognition Library (`speakerRecognitionEnabled`)

A persistent, on-device library of known people (name + voiceprint) so the same speaker is auto-named across meetings. Opt-in (`AppSettings.speakerRecognitionEnabled`), gated by `diarizationEnabled`, and **Power-User-only** for management.

- **Why FluidAudio**: SpeakerKit (Pyannote v4, argmax-oss-swift 1.0) does **not** expose voice embeddings, so recognition routes diarization through **FluidAudio's** `DiarizerManager` (Pyannote + WeSpeaker), whose `TimedSpeakerSegment` carries `embedding: [Float]`. New helper service `FluidAudioDiarizationService` (in `dBriefMLHost`) loads audio via `AudioConverter().resampleAudioFile`, runs `performCompleteDiarization`, and returns `DiarizedTurn`s with embeddings. Wire path: `MLRequest.diarizeWithEmbeddings(path:)` → `MLOrchestrator.diarizeWithEmbeddings` → `.diarizeResult([DiarizedTurn])`. `DiarizedTurn` gained an optional `embedding: [Float]?` (Codable back-compat; SpeakerKit path leaves it `nil`). App proxy: `LocalAIPluginService.diarizeWithEmbeddings(fileURL:)`.
- **Matching**: pure, unit-tested `SpeakerRecognizer` in `dBriefWire` (cosine similarity, per-speaker centroid, threshold) — the recognition analogue of `SpeakerMerge`.
- **Storage**: `SpeakerLibraryStore` actor (mirrors `InsightsStore`) persists one global `speaker-library.json` under `~/Library/Application Support/com.dbrief.app/` with `0600`/`0700` permissions. Models: `SpeakerLibrary` / `KnownSpeaker` (name + multiple `[[Float]]` samples → centroid). App-side coordinator `SpeakerRecognitionService` (`@MainActor @Observable`, owned by `AppContext`) does matching against the user threshold and mediates enrollment.
- **Flow**: when enabled, in-engine diarization is gated off and `RecordingManager.applySpeakerRecognition` runs the embedding pass for **all** engines, merges speakers via `SpeakerMerge`, and resolves names through `RichTranscriptBuilder.build(…, recognizedNames:)` (recognized name > ordinal participant > raw id). Embedding turns are held transiently on `Recording.pendingEnrollmentTurns` (never persisted). Manual "Detect Speakers" in `TranscriptDetailView` uses the embedding pass when recognition is on.
- **Enrollment**: explicit opt-in — "Remember this person" in the transcript rename sheet captures that speaker's centroid embedding and saves it. Managed in **Settings → Speaker Library** (`SettingsSpeakerLibraryTab`: enable, threshold, list/rename/delete, "Forget all speakers").
- **Privacy**: voiceprints are biometric — opt-in, on-device only, never in transcripts/markdown/integrations (only resolved names), one-way vectors, fully purgeable.
