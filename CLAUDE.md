...
- **SettingsView** — sidebar-based settings window with tabs:
  ...
  - **Benchmark** (`SettingsBenchmarkTab`) — hidden unless `powerUserMode` is enabled; hosts `ModelPerformanceView` (the global per-model timing aggregate from `ModelPerformanceStore`). Each transcription card shows **two** speeds — pure model **inference** (`TranscriptionResult.inferenceTime`, reported by the WhisperKit helper) and **end-to-end** (the full transcription step, including model load/prewarm, IPC, audio decode, diarization) — plus the load/overhead delta. Lives here because the data is overall, not tied to a single transcript — it is **not** in the transcript window toolbar. Reads the store via `@Environment(AppContext.self)` (the settings window injects `context`).
