...
- **Post-hoc speaker assignment**: Clicking a speaker label on a turn card in `TranscriptWindowView` opens `SpeakerAssignPicker` — a popover listing known people (existing transcript speakers, `recording.participants`, and calendar attendees) with a color dot and checkmark on the current speaker, plus an inline "Add someone…" field. Choosing a different person triggers a scope step when the speaker has other segments: **This turn** vs **All N from "<name>"**; single-segment speakers skip the scope step. Applied by the pure, unit-tested `SpeakerReassignment` (`Services/SpeakerReassignment.swift`), which rewrites `RichSegment.speakerId` over the chosen segment ids, drops orphaned `SpeakerLabel`s, transfers `meSpeakerId` if its speaker is fully merged away, and reuses an existing label id when the new name case-insensitively matches one. Persisted via `TranscriptStore` (`.richtranscript.json` sidecar). This picker subsumes free-text rename — entering a new name in "Add someone…" with scope "All" is equivalent to the former rename flow.
