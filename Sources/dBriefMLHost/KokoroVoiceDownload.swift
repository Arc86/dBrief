...
/// Additional English packs, converted from hexgrad/Kokoro-82M (Apache-2.0).
/// https://huggingface.co/txgsync/kokoro-82m-coreml-ane
/// Cache at FluidAudio's English repo root so its existing loader can use them.
enum KokoroVoiceDownload {
    static let revision = "89346e8f1f0025228eb3dc37b7e6febada07606a"
    ...
    static func ensure(
        _ voice: KokoroVoice,
        repoDirectory: URL,
        download: (URL) async throws -> Data = fetch
    ) async throws {
        guard let checksum = checksums[voice.rawValue] else {
            throw WireError(kind: .generic, message: "Kokoro: unsupported English voice.")
        }
        let target = repoDirectory.appendingPathComponent("\(voice.rawValue).bin")
        if let cached = try? Data(contentsOf: target), valid(cached, checksum: checksum) { return }

        let url = URL(string: "https://huggingface.co/txgsync/kokoro-82m-coreml-ane/resolve/\(revision)/voices/\(voice.rawValue).bin")!
        let data = try await download(url)
        try Task.checkCancellation()
        guard valid(data, checksum: checksum) else {
            throw WireError(kind: .generic, message: "Kokoro: the downloaded \(voice.displayName) voice failed verification. Please try again.")
        }
        try FileManager.default.createDirectory(at: repoDirectory, withIntermediateDirectories: true)
        try data.write(to: target, options: .atomic)
    }

    private static func valid(_ data: Data, checksum: String) -> Bool {
        data.count == 522_240 && SHA256.hash(data: data).map { String(format: "%02x", $0) }.joined() == checksum
    }

    private static func fetch(_ url: URL) async throws -> Data {
        let (data, response) = try await URLSession.shared.data(from: url)
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            throw WireError(kind: .generic, message: "Kokoro: voice download failed. Please try again.")
        }
        return data
    }
}
